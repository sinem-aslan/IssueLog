...
<nav x-data="{ open: false }" style="background:#dce1ea;" class="border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <!-- kullanıcı isminin bulunduğu buton -->
                        <button style="background:#dce1ea;"
                            class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div x-data="{{ json_encode(['name' => auth()->user()->name, 'surname' => auth()->user()->surname]) }}" x-text="'Merhaba, ' + name + ' ' + surname"
                                x-on:profile-updated.window="name = $event.detail.name; surname = $event.detail.surname">
                            </div>
                            <!--name yanında bulunan aşağı ok ikonu -->
                            <div class="ms-1">
